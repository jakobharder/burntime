...
internal static class CampEconomy
{
    ...
    // After supplying its stationed guards, a mature waypoint should still
    // produce one daily unit for each member of the normal two-person travel
    // group. Until the empire has such a camp, preserve at least one unit so an
    // emergency demobilization cannot consume the fledgling camp's whole output.
    internal const int TravelSupplySurplusTarget = 2;
    ...
    public static bool IsWellEstablished(Location camp) => camp.Player != null &&
        ...
        WaterSurplusPerDay(camp) >= TravelSupplySurplusTarget &&
        FoodSurplusPerDay(camp) >= TravelSupplySurplusTarget;
    ...
    public static int WantedSupplySurplus(ClassicAiState state) =>
        state.RootGame.World.Locations.Any(location =>
            location.Player == state.Player && IsWellEstablished(location))
            ? TravelSupplySurplusTarget
            : 1;
    ...
    public static bool CanProvisionGroupWater(
        Location camp,
        int groupSize,
        int reserveDays = 3)
    {
    ...
    public static bool IsTravelWaterBottleneck(Location camp, int groupSize = 2) =>
        camp.Player != null && !CanProvisionGroupWater(camp, groupSize);
        ...
        Production production)
    {
        if (AiTurnContext.For(state).Camps.TryGet(camp, out CampAssessment assessment) &&
            assessment.DesiredProductionTools.TryGetValue(production, out int desired))
            return desired;

        int calculated = CalculateDesiredProductionToolCount(state, camp, production);
        if (AiTurnContext.For(state).Camps.TryGet(camp, out assessment))
            assessment.DesiredProductionTools[production] = calculated;

        return calculated;
    }

    internal static int CalculateDesiredProductionToolCount(
        ClassicAiState state,
        Location camp,
        Production production)
    {
        ...
        bool worthwhileRedistribution = state.RootGame.World.Locations
            ...
            .Where(location => location != camp && location.ValidProductions.Contains(production) &&
                ...
                Trading.ProductionTradePriority(production) >= location.ValidProductions
                    .Where(candidate => ProductionToolCount(location, candidate) > 0)
                    .Select(Trading.ProductionTradePriority)
    ...
    public static bool IsFoodStockCapped(Location camp) => camp.Production != null &&
        camp.GetCurrentProductionStockCount() >= Location.MaxStockFood;
}
