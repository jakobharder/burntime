...
internal static partial class ExpansionPlanning
{
        ...
        DecisionContext context)
    {
        ...
        if (!state.HasSettlementPlan || target == null ||
            !AttackPlanning.IsHostile(context.Current, context.Player))
            return;
        ...
        AiPolicy policy)
    {
        ...
        if (target == null && !(canClaimCurrent && state.StrategicTarget == null))
        {
            ...
            if (AttackPlanning.IsHostile(target, context.Player))
            {
    ...
    public static bool TryClaimCurrentAsLocalOpportunity(ClassicAiState state)
    {
        ...
        GroupManagement.ProvisionGroupFromCampSurplus(state, current);
    ...
    public static void AddCandidates(
        ...
        List<AiDecision> candidates)
    {
        if (state.PostCapturePauseTurns > 0 && !context.CriticalSupplies && !plan.PreparingAttack)
        {
            state.PostCapturePauseTurns--;
            candidates.Add(new AiDecision(
                AiAction.Wait,
                900,
                Reason: "post-capture pause"));
        }
        ...
        else if (AttackPlanning.CanAdvancePlan(state, plan.Target, policy))
        {
            // CanAdvancePlan validates either the complete hostile route or a
            // supplied first leg to a friendly waypoint. Preserve that staged
            // approval instead of asking the generic travel helper to require
            // supplies for the complete route a second time.
            RouteFinder.Route? route = RouteFinder.Find(
                context.Player, context.Current, plan.Target);
    ...
    public static bool CanBootstrapCamp(ClassicAiState state, Location location)
    {
        ...
        if (!CampEconomy.HasFoodProductionPotential(location))
            return false;
        if (ReinforcementPlanning.IsThreatened(state, location))
            return false;
    ...
    static bool HasNeighboringFoodSupport(ClassicAiState state, Location location)
    {
        ...
        for (int index = 0; index < location.Neighbors.Count; index++)
        {
            ...
            if (route == null || !TravelSupplies.CanSurviveRecoveryRoute(state.Player, route))
                continue;
    ...
    {
        if (!Trading.HasNeutralExpansionOpportunity(state))
            return false;
        ...
        AiPolicy policy)
    {
        ...
        return AttackPlanning.ValidatePersistentTarget(state, context, policy, target, route);
    }
        ...
        AiPolicy policy)
    {
        if (!HasOwnedCamp(state))
            return SelectFirstSettlementTarget(state, context, policy);

        List<(Location Location, float Score)> targets = new();
        bool attackRecruitmentDeferred = state.IsAttackRecruitmentDeferred(policy);
        foreach (Location location in state.RootGame.World.Locations)
        {
            if (location.IsCity || TerritorialTargetDeferrals.IsDeferred(state, location))
                continue;
            ...
            else if (!attackRecruitmentDeferred &&
                AttackPlanning.HasGroupWeapon(context.Player) &&
                AttackPlanning.IsHostile(location, context.Player) &&
                AttackPlanning.IsTerritorialFrontierTarget(state, location) &&
                AttackPlanning.IsTargetAllowed(state, location, policy))
            {
                ...
                float strategicBonus = (ReinforcementPlanning.IsStrategicLocation(state, location) ||
    ...
    static Location? SelectFirstSettlementTarget(
        ClassicAiState state,
        DecisionContext context,
        AiPolicy policy)
    {
        ...
        {
            AiTelemetry.Report(context.Player,
                "first-settlement plan blocked: neutral expansion is disabled");
            return null;
        }
        ...
        var candidates = state.RootGame.World.Locations
            ...
            {
                Location = location,
                Route = location == context.Current
                    ? null
                    : RouteFinder.Find(context.Player, context.Current, location)
            })
            .Where(candidate => candidate.Location == context.Current ||
                candidate.Route != null && HasSettlementRouteSupplies(
                    state, context, candidate.Location, candidate.Route))
            .ToList();

        // Good food sites bootstrap a useful territory. A maggot-only camp is
        ...
        Location target)
    {
        ...
        int score = 110;
        if (AttackPlanning.CanAdvancePlan(state, target, policy))
            score += 180;
    ...
    static bool HasReachableAcceptableFirstCamp(ClassicAiState state, DecisionContext context) =>
        ...
        RouteFinder.Route route)
    {
        if (context.Player.Group.Count == 1)
        {
            ...
            // The solo leader cannot advance on a neutral target via ExpansionPlanning.
            return true;
        }
        ...
        return returnRoute != null && TravelSupplies.HasSettlementRoundTripFood(
        ...
        while (queue.Count > 0)
        {
            ...
            for (int index = 0; index < location.Neighbors.Count; index++)
            {
                if (location.WayLengths[index] <= 0)
                    continue;
                Location neighbor = location.Neighbors[index];
                if (!AttackPlanning.IsHostile(neighbor, player) && visited.Add(neighbor))
    ...
    static bool HasExpansionRouteSupplies(
        ...
        RouteFinder.Route route)
    {
        if (TravelSupplies.HasTerritorialRouteSupplies(
        ...
        return leg != null && TravelSupplies.HasRouteSupplies(
