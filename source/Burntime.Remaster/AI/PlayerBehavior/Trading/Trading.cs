...
internal static partial class Trading
{
        ...
        List<AiDecision> candidates)
    {
        ...
        bool expansionNeedsEquipment = ExpansionPlanning.NeedsExpansionTool(state);
        ...
        bool fundedSnakeTrapCampaign = EconomicSupport.IsSavingForSnakeTrap(state) &&
            Trading.HasAffordableHighReturnTradeCargo(state);
        ...
        bool shouldContinueTrading = Trading.ShouldContinueTrading(state);
        ...
        else if (shouldVisitTrader)
        {
            Location? tradeCity = bestTradeCity ??
                AiTurnController.FindNearestCity(state);
            ...
            if (!preparingAttack && Trading.ShouldReduceTradeCaravan(state))
            {
                candidates.Add(new AiDecision(
            ...
            else if (Trading.ShouldFillCityCaravanBeforeDeparture(state, bestTradeCity))
            {
                candidates.Add(new AiDecision(
        ...
        CampDelivery.AddDeliveryCandidate(
            state, candidates,
            System.Math.Min(preparedEconomyScore,
        ...
        if (deliveryMilliseconds >= 50)
            AiTelemetry.Report(state.Player,
                $"Trading.AddCandidates took {deliveryMilliseconds} ms: expansion equipment " +
