...
class RenderDevice : IDisposable
{
    ...
    Engine engine;
    Burntime.Platform.Resource.ResourceManager resourceManager;
    ...
    public bool Initialize(RenderForm Form)
    {
        ...
        Log.Info("Virtual resolution: " + engine.Resolution.Game.x + "x" + engine.Resolution.Game.y);
        Log.Info("Scale factor: " + engine.Resolution.Scale.x.ToString("0.00") + "x" + engine.Resolution.Scale.y.ToString("0.00"));
    ...
    {
        SpriteFrame.EmptyTexture = new Texture(device, 1, 1, 1, Usage.None, Format.A8R8G8B8, Pool.Managed);
        SlimDX.DataRectangle dr = SpriteFrame.EmptyTexture.LockRectangle(0, LockFlags.Discard);
        dr.Data.Write<uint>(0xff000000);
        SpriteFrame.EmptyTexture.UnlockRectangle(0);
    ...
    void Render(GameTime RenderTime)
    {
        ...
        BlendOverlay.BlockFadeOut = engine.IsLoading || BlendOverlay.Block;
        BlendOverlay.Render(RenderTime, spriteRenderer);
        if (engine.MusicBlend)
        {
            engine.Music.Volume = 1 - BlendOverlay.BlendState;
        }
        else
        {
